...
export default function HotelCards({
  ...
  useEffect(() => {
    const fetchHotelData = async () => {
      ...
      try {
        const cityName = location ? location.toLowerCase() : "chennai";
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/restaurants/${cityName}`
        );
    ...
    <>
      <Grid2 container gap={1}>
        {filteredHotels.map((eachHotel) => {
          const { id, name, location, priceDetail, ratings, tags, image } =
            eachHotel;

          return (
            <Grid2
              key={id}
              onClick={() => handleBookingModelOpen(eachHotel)}
