...
export default function BookingModal({ handleClose, modalState }) {
  ...
  const getHotelDetails = async (city, hotelId) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}restaurants/${city}/${hotelId}`
      );
  ...
  const getReviews = async (hotelId) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}get-reviews/${hotelId}`
      );
    ...
    if (
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}create-booking`, // Use the backend URL from the environment
          {
            userId,
            userName,
            userEmail,
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}submit-review`,
        {
          hotelId: id,
          userId,
          name,
  ...
  const handleAdminReply = async (reviewId) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}admin/reply`,
        {
          reviewId,
          adminReply: reply,
        }
              ...
              <Grid container spacing={1}>
                {timeSlots.map((eachTime) => (
                  <Grid item key={eachTime}>
                    <Chip
                      color={
                        bookingState.selectedTime === eachTime
                          ? "primary"
                          : "default"
                      }
